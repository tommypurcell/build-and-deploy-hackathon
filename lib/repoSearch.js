...
const SOURCE_FILE_RE = /\.(js|jsx|ts|tsx|mdx)$/i;
...
function findSnippet(content, query, queryTokens) {
  const text = String(content || "");
  const lower = text.toLowerCase();
  let index = lower.indexOf(String(query || "").toLowerCase());
  ...
  if (index === -1) {
    return compactText(text.slice(0, 280), MAX_LINE_LEN);
  }

  const start = Math.max(0, index - 120);
  const end = Math.min(text.length, index + 260);
  return compactText(text.slice(start, end), MAX_LINE_LEN);
}

function scoreFileMatch(filePath, content, query, queryTokens) {
  const pathText = String(filePath || "");
  const rawText = `${pathText}\n${String(content || "")}`;
  const lowerPath = pathText.toLowerCase();
  const lowerText = rawText.toLowerCase();
  const collapsedQuery = collapseSearchText(query);
  const collapsedPath = collapseSearchText(pathText);
  const collapsedText = collapseSearchText(rawText);
  const pathTokens = tokenizeSearchText(pathText);
  const contentTokens = tokenizeSearchText(content).slice(0, 300);

  let score = 0;

  if (lowerPath.includes(String(query || "").toLowerCase())) score += 28;
  if (lowerText.includes(String(query || "").toLowerCase())) score += 16;
  if (collapsedQuery && collapsedPath.includes(collapsedQuery)) score += 24;
  if (collapsedQuery && collapsedText.includes(collapsedQuery)) score += 14;

  for (const token of queryTokens) {
    if (pathTokens.some((candidate) => tokenMatches(token, candidate))) {
      score += 9;
      continue;
    }

    if (contentTokens.some((candidate) => tokenMatches(token, candidate))) {
      score += 4;
    }
  }
...
async function buildRepoContext(options = {}) {
  ...
  const candidateKeyFiles = [
    ...
    "lib/repoSearch.js",
    ...filePaths.filter((path) => path.startsWith("src/") && SOURCE_FILE_RE.test(path)).slice(0, 4),
    ...filePaths.filter((path) => path.startsWith("demo/") && SOURCE_FILE_RE.test(path)).slice(0, 2),
...
export async function searchRepo(query, options = {}) {
  ...
  try {
    ...
    for (const file of textFiles) {
      ...
      if (!content) continue;
      const score = scoreFileMatch(file.path, content, q, queryTokens);
      if (score <= 0) continue;

      matches.push({
        file: file.path,
        score,
        snippet: findSnippet(content, q, queryTokens),
      });
    }
