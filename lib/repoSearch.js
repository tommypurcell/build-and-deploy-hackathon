...
function getGitHubConfig() {
  ...
  if (parsedFromUrl) {
    return { token, owner: parsedFromUrl.owner, repo: parsedFromUrl.repo };
  }
...
/**
 * Fallback: Get full repository structure as JSON
 * Returns a comprehensive JSON structure of all text files in the repo
 */
export async function getRepoStructureJSON() {
  try {
    const { token, owner, repo } = getGitHubConfig();
    const tree = await fetchRepoTree(owner, repo, token);
    ...
    const textFiles = tree
      .filter((item) => item.type === "blob" && isTextFile(item.path))
      .slice(0, 50); // Limit to 50 files to avoid huge payloads
    ...
    return JSON.stringify({
      error: true,
      message: `Failed to fetch repo structure: ${e.message}`,
      suggestion: "Check GITHUB_TOKEN, GITHUB_REPO_OWNER, and GITHUB_REPO_NAME environment variables.",
    }, null, 2);
  }
}
